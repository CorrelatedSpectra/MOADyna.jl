...
`Hilbert` and `Restriction`s from [`MOAD.Algebra`](@ref), produces an
...
re-exported from the umbrella package, so users typically write `using MOAD`
instead of reaching for `MOAD.Bases` directly.
