...
- Representation theory: [`subduce`](@ref), [`project`](@ref),
  [`wignerd`](@ref).
- Crystal field: [`expand_clm`](@ref MOADyna.PointGroups.expand_clm), [`expand_clm_central`](@ref MOADyna.PointGroups.expand_clm_central),
  [`nparams`](@ref).
...
- `LiftedRep`, `lift` — single-particle rotation matrices per
  ...
  built from these and exposed as `MOADyna.classify_state(ψ, basis, m, G)`
  (in `MOADyna.Diagnostics`, which can reach the `Shells`/`Bases` layers
...
# `rotate` is shared with MOADyna.Algebra; extend the same generic function so
# `MOADyna` re-exports a single name with methods on both `PointGroup` (here)
# and `OperatorSum` (Algebra). Algebra is included first in MOADyna.jl.
...
# single-particle primitive behind `MOADyna.classify_state(ψ, basis, m, G)`
# (the many-body Fock-space apply lives in `MOADyna.Diagnostics`). Reach the
# primitive via `MOADyna.PointGroups.lift` / `MOADyna.PointGroups.LiftedRep`.
