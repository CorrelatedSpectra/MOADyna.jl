...
(`MOAD.Algebra`), comparing the Tier-2 canonicalized `OperatorSum` output
...
| MOAD   | 1-indexed (or whatever the user assigns) | ascending | ascending |
...
| Term | Quanty (descending, 0-indexed) | MOAD (ascending, 1-indexed) |
|---|---|---|
...
`z_only_compare.jl` builds the same Hamiltonian in MOAD, parses the
...
This validates that MOAD's Tier-2 algebra produces algebraically identical
...
- restrictions (`MOAD.Bases` is in; matrix-element comparison vs a
