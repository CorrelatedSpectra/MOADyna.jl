# Validation: build the same H as test_print_hamiltonian.Quanty in MOADyna
# and check term coefficients.

using MOADyna

# Quanty: NF=4, modes 0..3 = (orb 0 up, orb 0 dn, orb 1 up, orb 1 dn).
# In MOADyna, single FermionSite{4} with modes labeled 1..4.
# Mapping (1-indexed in MOADyna): 1↔0_up, 2↔0_dn, 3↔1_up, 4↔1_dn

s = FermionSite{4}(:s)

# Hopping: 1.5 * (cdag(0) c(2) + cdag(1) c(3) + h.c.) in Quanty notation
# In MOADyna (1-indexed): 1.5 * (cdag(s,1) c(s,3) + cdag(s,2) c(s,4) + h.c.)
t_hop = 1.5
...
println("  C 2 A 0 | +1.5")
println("  C 3 A 1 | +1.5")
println("MOADyna H_hop:  ", length(H_hop), " terms (should be 4)")
println()
println("Quanty H_U term (length 4):")
println("  C 1 C 0 A 1 A 0 | -4.0")
println("    [cdag(1) cdag(0) c(1) c(0) — Quanty stores DESCENDING]")
println("MOADyna H_U:    ", length(H_U), " term (should be 1)")
println("MOADyna's canonical form is ASCENDING, so the chain is")
