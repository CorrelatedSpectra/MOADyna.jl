...
expxas = readdlm(joinpath(pkgdir(MOAD), "docs", "src", "assets", "nio", "nio_xas_L23_exp.dat");
                 comments = true)

plt = plot(photon, Iiso ./ maximum(Iiso);
           label = "MOAD (shifted by $(round(Eshift; digits = 1)) eV)", lw = 2,
...
tabulated Ni ``3d`` Hartree–Fock radial function. That data file is generated by MOAD itself —
...
```@example nio
# Ni 3d Hartree–Fock radial function (MOAD-generated; column 7 "3d" is P(r)=r·R(r)).
M     = readdlm(joinpath(pkgdir(MOAD), "docs", "src", "assets", "nio", "Ni_3d_HF_radial.dat"),
                '\t'; skipstart = 1)
rgrid = Float64.(M[:, 1])
P3d   = Float64.(M[:, 7])
...
expnixs = readdlm(joinpath(pkgdir(MOAD), "docs", "src", "assets", "nio", "nio_nixs_dd_exp.dat");
                  comments = true)
...
[`convert_energy`](@ref)`(Tkelvin, :K => :eV)` — `MOAD.Units` provides this
