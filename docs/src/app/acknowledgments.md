...
MOAD builds on the Julia numerical ecosystem. The packages below are runtime
dependencies — code that ships and runs as part of MOAD — and we are grateful to
...
- **[KrylovKit.jl](https://github.com/Jutho/KrylovKit.jl)** (Jutho Haegeman and
  ...
  primitives behind MOAD's exact diagonalization and the block-Lanczos
...
MOAD also ships an optional plot recipe for
**[Plots.jl](https://github.com/JuliaPlots/Plots.jl)**, wired through a package
extension so it loads only when the user has Plots installed (MOAD itself does
...
For arbitrary physical constants beyond MOAD's small energy-unit helper
([`MOAD.Units`](@ref)), the idiomatic source is
[PhysicalConstants.jl](https://github.com/JuliaPhysics/PhysicalConstants.jl)
(CODATA, `Unitful`-typed); MOAD does not depend on it.

## Citing

**Citing the software.** Please cite the version you used; machine-readable
metadata is in
[`CITATION.cff`](https://github.com/CorrelatedSpectra/MOAD.jl/blob/main/CITATION.cff).
...
MOAD grew out of many years of working with
...
core-level spectroscopy (the author is a Quanty coauthor). MOAD is an independent
